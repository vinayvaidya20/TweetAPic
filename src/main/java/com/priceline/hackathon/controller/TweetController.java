...
public class TweetController {
	...
	public String index(RedirectAttributes redirectAttributes) {
		...
		if(null != emailList && !emailList.isEmpty()) {
			redirectAttributes.addFlashAttribute("message", "Had a great trip at "+emailList.get(emailList.size() - 1).getTrip().getCity()+".. Thank you Priceline! :) #everyDealIsABigDeal #ThankYouPriceline");
		} else
			redirectAttributes.addFlashAttribute("message", "Had a great trip at NYC.. Thank you Priceline! :) #everyDealIsABigDeal #ThankYouPriceline");
	...
	public ResponseEntity<?> singleFileUpload(@RequestParam(required = false, value = "file") MultipartFile file,
		...
		try {
			...
			if (tweetResponse) {
				...
				readLock.lock();
				try {
					user = emailList.get(emailList.size() - 1);
				} finally {
					readLock.unlock();
				}
				Deal deal = new Deal();
